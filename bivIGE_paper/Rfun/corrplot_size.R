                    ...
                    ...)
{
  ...
  #print(paste("method = ",method, "type = ",type, "order = ",order, "hclust.method = ",hclust.method, "addshade = ",addshade, "insig = ",insig,  "plotCI = ",plotCI))}
  #method =  "square"; type =  "full"; order =  "original"; hclust.method =  "complete"; addshade =  "negative"; insig =  "pch"; plotCI =  "n"
  ...
  getPos.NAs = function(mat) {
    ...
    PosName = ind #add H
    PosName[, 1] = colnames(mat)[ind[, 2]] #add H
    PosName[, 2] = rownames(mat)[ind[, 1]] #add H
    #return(Pos)
    return(list(Pos, Dat, PosName)) # add H
  }
  ...
  if (any(is.na(corr)) && is.character(na.label)) {
    PosNA = getPos.NAs(corr)[[1]] #[[1]] add H
    DatNA = getPos.NAs(corr)[[2]] # this used only in output # add H
    PosNameNA = getPos.NAs(corr)[[3]] # this used only in output # add H
  } else {
    # explicitly set to NULL to indicate that NA labels are not going to be
    # rendered
    PosNA = NULL
    DatNA = NULL # this used only in output # add H
    PosNameNA = NULL # this used only in output # add H
  }
  #print(PosNA)
  ...
  if (method == 'circle' && plotCI == 'n') {
    ...
    if(is.null(size_vector)){
      symbols(Pos, add = TRUE,  inches = FALSE,
              #circles = asp_rescale_factor * 0.9 * abs(DAT) ^ 0.5 / 2, # dimension based on absolute value of the corr
              circles = asp_rescale_factor * 0.9 * rep(1, length(DAT)) ^ 0.5 / 2, # dimension the same for all
              fg = col.border, bg = col.fill)
    }else{
      if(any(size_vector < 0) ){stop("can't plot negative size, check 'size_vector'")}
      symbols(Pos, add = TRUE,  inches = FALSE,
              circles = asp_rescale_factor * 0.9 * size_vector ^ 0.5 / 2, # line 630}
              fg = col.border, bg = col.fill)
    }
  }
  ...
  if (method == 'square' && plotCI == 'n') {
    draw_method_square(Pos, DAT, asp_rescale_factor, col.border, col.fill, size_vector)
  ...
  if (!is.null(p.mat) && insig != 'n') {
    ...
    posName.pNew = getPos.Dat(p.mat)[[3]] # this used only in output # add H
    
    if (any(is.na(p.mat))){
      posNA.pNew     = getPos.NAs(p.mat)[[1]] # this used only in output # add H
      NA.pNew        = getPos.NAs(p.mat)[[2]] # this used only in output # add H
      posNameNA.pNew = getPos.NAs(p.mat)[[3]] # this used only in output # add H
    }else{
      posNA.pNew     = NULL 
      NA.pNew        = NULL
      posNameNA.pNew = NULL
    ...
    # check there are significant points # added by Helene, 2 April 2025
    if (!any(pNew < sig.level)) {
      print("no significant points found")
    } else if (insig == 'label_sig') {
      ...
      if (length(sig.level) == 1) {
        #print(pNew)# added 2 April 2025
        #print(which(pNew < sig.level)) # added 2 April 2025
  ...
  corrPos = data.frame(PosName, Pos, DAT) # DAT = getPos.Dat(corr)[[2]]
  colnames(corrPos) = c('xName', 'yName', 'x', 'y', 'corr') 
  
  if(!is.null(PosNA)){
    corrNAs = data.frame(PosNameNA, PosNA, DatNA) # add H
    colnames(corrNAs) = c('xName', 'yName', 'x', 'y', 'corr') # add H
    corrPos = rbind(corrPos, corrNAs)
  }
  
  if(!is.null(p.mat)) {
    pPos = data.frame(posName.pNew, pos.pNew, pNew) # add H
    colnames(pPos) = c('xName', 'yName', 'x', 'y', 'p.value') 
    
    if(!is.null(posNA.pNew)){
      pNAs = data.frame(posNameNA.pNew, posNA.pNew, NA.pNew) # add H
      colnames(pNAs) = c('xName', 'yName', 'x', 'y', 'p.value') # add H
      pPos = rbind(pPos, pNAs) # add H
    }
    
    corrPos = merge(corrPos, pPos, by=c("xName", "yName",  "x",  "y"))
...
#' @noRd
draw_method_square = function(coords, values, asp_rescale_factor, fg, bg, size_vector) {
  if(is.null(size_vector)){
    ...
    if(any(size_vector < 0) ){stop("can't plot negative size, check 'size_vector'")}
    #print(dim(coords))
    #print(length(size_vector))
    #print(head(asp_rescale_factor))
    #print(length(asp_rescale_factor * size_vector ^ 0.5))
    symbols(coords, add = TRUE, inches = FALSE,
            squares = asp_rescale_factor * size_vector ^ 0.5, # line 630}
            bg = bg, fg = fg)
  }
}
...
####################################################################
###### TRY THIS VERSION ##################
##### Taken from here: https://github.com/mkanai/ldsc-corrplot-rg/tree/master?tab=readme-ov-file ########
#####                  - plot_corrplot_rg.R
##################################
####### their corrplot version is here:
######## https://github.com/mkanai/corrplot/tree/master/
