#runs the VD with the correlation rho between DGE and IGE constrained to be +/- 1
...
if __name__=='__main__':
    ...
    #lr is low rank. equivalent to imposing correlation 1 or -1 between DGE and IGE.
    vc_lr = DirIndirVD(vc_init_type = 'lowrank', vc_init = vc_ff, pheno = doto['pheno'],pheno_ID = doto['pheno_ID'],covs = doto['covs'], covs_ID = doto['covs_ID'], covariates_names = doto['covariates_names'], kinship_all = doto['kinship_full'], kinship_all_ID = doto['kinship_full_ID'], cage_all = doto['cage_full'], cage_all_ID = doto['cage_full_ID'], DGE = (DGE is not None), IGE = (IGE is not None), IEE = (IEE is not None), cageEffect = (cageEffect is not None), calc_ste=False, subset_IDs = doto['subset_IDs'], SimplifNonIdableEnvs = False)
    rv=vc_lr.getOutput()
